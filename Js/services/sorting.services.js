...
class Sort {
  ...
  async removeAllEmptyFolders() {
    const allFiles = await globby(['**/**/*', '!services', '!storage', '!node_modules', '!Sorted files'], { onlyDirectories: true, dot: true })
  ...
  async checkFiles() {
    const allFiles = await globby(['**/**/*', '!services', '!storage', '!node_modules', '!Sorted files'], { onlyFiles: true, dot: true });
    let paths = []
